...
//     void Update()
//     {
//         // InverseKinematics3D(Target.position);
//     }
...
//         sensor.AddObservation(Target.position.normalized); // Target Position [3]
//         sensor.AddObservation(transform.position.normalized); // Robot Position [3]
//         sensor.AddObservation(EndEffector.position.normalized); // End Effector Position [3]
//         sensor.AddObservation((Target.position - EndEffector.position).normalized); // Vector to Target [3]
//         sensor.AddObservation(Vector3.Distance(EndEffector.position, Target.position) / (maxRange)); // Distance to Target [1]
...
//         float angle0 = Mathf.Clamp(Joint0.localRotation.eulerAngles.y + angles[0] * Time.deltaTime * rotateSpeed, -180f, 180f);
//         float angle1 = Mathf.Clamp(Joint1.localRotation.eulerAngles.z + angles[1] * Time.deltaTime * rotateSpeed, -90f, 90f);
//         float angle2 = Mathf.Clamp(Joint2.localRotation.eulerAngles.z + angles[2] * Time.deltaTime * rotateSpeed, -180f, 180f);
//         float angle3 = Mathf.Clamp(Joint3.localRotation.eulerAngles.z + angles[3] * Time.deltaTime * rotateSpeed, -180f, 180f);
...
//         float distance_to_target = Vector3.Distance(EndEffector.position, Target.position);
...
//     {
//         float bonus = 1f * (int)Mathf.Clamp01(Vector3.Dot(Target.up.normalized, EndEffector.up.normalized)); // Bonus for alignment
//         AddReward(1f + bonus);
//         EndEpisode();
//     }
...
//     private void InverseKinematics3D(Vector3 targetPos)
//     {
//         Vector3 dirFromBase = targetPos - Joint0.position;
...
//         Vector3 joint1Local = invBaseRot * (Joint1.position - Joint0.position);
//         Vector3 joint3Local = invBaseRot * (Joint3.position - Joint0.position);
//         Vector3 targetLocal = invBaseRot * (targetPos - Joint0.position);
...
//     }
//     private void InverseKinematics2D()
//     {
//         Vector3 targetPos = Target.position;
//         float x = targetPos.x - Joint1.position.x;
//         float y = targetPos.y - Joint1.position.y;

//         float maxLength = link1Length + link2Length + link3Length - 1e-4f;
//         Vector3 targetVector = targetPos - Joint1.position;

//         if (targetVector.magnitude > maxLength)
//         {
//             Vector3 clampedTargetVector = targetVector.normalized * maxLength;
//             x = clampedTargetVector.x;
//             y = clampedTargetVector.y;
//         }
...
//         // float phi_e = Target.eulerAngles.z * Mathf.Deg2Rad;
//         float phi_e = Mathf.Atan2(targetPos.y - Joint1.position.y, targetPos.x - Joint1.position.x);
...
//         // Eigenen
//         float theta2 = Mathf.PI - Mathf.Acos((L1 * L1 + L2 * L2 - (xw * xw) - (yw * yw)) / (2 * L1 * L2));
//         float theta1 = Mathf.Atan2(y, x) - Mathf.Acos((xw * xw + yw * yw + L1 * L1 - L2 * L2) / (2 * L1 * Mathf.Sqrt(xw * xw + yw * yw)));
//         float theta3 = phi_e - theta1 - theta2;
