...
public class ArmAgent_IK_TR : Agent
{
    [Header("Environment Connection")]
    [Tooltip("Drag the SortingEnvironment object here.")]
    [SerializeField] private SortingEnvironment_TR environment;
    ...
    [Header("Visual Tracking")]
    [SerializeField] private bool enableVisualTracking = false;
    [SerializeField] private VisualTrackingController visualTrackingController;
    ...
    public override void Initialize()
    {
        // 1. Check Environment
        if (environment == null)
            environment = GetComponent<SortingEnvironment_TR>() ?? FindFirstObjectByType<SortingEnvironment_TR>();

        if (environment == null)
            Debug.LogError($"<color=red><b>[ArmAGent_IK_TK] FATAL ERROR:</b> No SortingEnvironment found!</color>", this);
        ...
        if (ikController == null)
            Debug.LogError($"<color=red><b>[ArmAGent_IK_TK] ERROR:</b> RobotArm_IK_Controller not assigned!</color>", this);
    ...
    public override void OnEpisodeBegin()
    {
        ...
        previousDistanceToBottle = Vector3.Distance(currentTargetPosition, GetBottlePosition());
        previousDistanceToBin = environment.GetHorizontalDistanceToBin(GetBottlePosition());

        Debug.Log($"[ArmAGent_IK_TK] Episode Start. Reward: {GetCumulativeReward():F2}");
    }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(transform.InverseTransformPoint(GetBottlePosition()));
        ...
        sensor.AddObservation(GetBottlePosition() - endEffector.position);
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        float currentDistanceToBottle = Vector3.Distance(currentTargetPosition, GetBottlePosition());
        float currentDistanceToBin = environment.GetHorizontalDistanceToBin(GetBottlePosition());
        ...
        if (!IsHoldingObject() && (GetBottlePosition(true).y < (environment.bottleSpawnPoint.position.y - 0.4f)))
        {
    ...
    public void OnPartCollision(string hitTag)
    {
        ...
        {  
            Debug.Log($"<color=red>[ArmAGent_IK_TK] Collision ({hitTag}).</color>");
            AddReward(-1.0f); 
    ...
    */
    ...
    private void FinishEpisode(bool success, string reason = "")
    {
        ...
        Debug.Log($"<color={color}>[ArmAGent_IK_TK] Episode Finished. Lesson: {lesson} | Result: {result} | Reward: {reward:F2} | Steps: {steps} | Reason: {reason}</color>");
