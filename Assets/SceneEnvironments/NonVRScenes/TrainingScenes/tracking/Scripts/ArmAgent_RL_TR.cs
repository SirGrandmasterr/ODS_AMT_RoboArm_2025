...
public class ArmAgent_RL_TR : Agent
{
    ...
    public override void Initialize()
    {
        ...
        if (environment == null)
        {
            ...
            environment = GetComponent<SortingEnvironment_TR>();

            // Then try finding it globally (fallback)
            if (environment == null)
            {
                environment = FindFirstObjectByType<SortingEnvironment_TR>();
            }

            if (environment == null)
            {
                Debug.LogError($"<color=red><b>[ArmAgent_RL_TR] FATAL ERROR:</b> 'Environment' field is not assigned and no SortingEnvironment found in scene! Agent cannot function.</color>", this);
        ...
        {
            Debug.LogError($"<color=red><b>[ArmAgent_RL_TR] ERROR:</b> One or more Arm Joints (Transforms) are not assigned in the Inspector!</color>", this);
        }
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (environment.CurrentLessonNumber >= 2f)
        {
            ...
            if (!IsHoldingObject())
            {
                ...
                {
                    AddReward(5.0f);
                    Debug.Log($"<color=green>[ArmAgent_RL_TR] SUCCESS: Bottle placed in CORRECT bin! Reward: {GetCumulativeReward():F2}</color>");
                    EndEpisode();
                }
                else if (environment.CheckPlacementFailure())
                {
                    AddReward(-2.0f);
                    Debug.Log($"<color=yellow>[ArmAgent_RL_TR] FAIL: Bottle placed in WRONG bin. Reward: {GetCumulativeReward():F2}</color>");
                    EndEpisode();
                }
            }
        }
        ...
        if (!IsHoldingObject() && (environment.bottle.position.y < (environment.bottleSpawnPoint.position.y - 0.5f)))
        {
            ...
            if (!environment.IsInBinZone())
            {
                ...
                Debug.Log($"<color=red>[ArmAgent_RL_TR] FAILED: Bottle fell out of bounds. Reward: {GetCumulativeReward():F2}</color>");
                EndEpisode();
            }
        }
    }
    ...
    public void OnPartCollision(string hitTag)
    {
        ...
        {
            AddReward(-1.0f);
            Debug.Log($"<color=orange>[ArmAgent_RL_TR] COLLISION FAIL: Hit {hitTag}</color>");
            EndEpisode();
        }
    }
